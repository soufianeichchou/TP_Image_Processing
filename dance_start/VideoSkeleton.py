...
class VideoSkeleton:
        ...
        for i in range(video.getTotalFrames()):
            ...
            if (i % mod_frame == 0):
                ...
                if isSke:
                    ...
                    print("frame " + str(i) + "/" + str(
                        video.getTotalFrames()) + "   filename=" + filename_im + "  save=" + filename_imsave + " sizeof=" + str(
                        sys.getsizeof(self.ske)))
            #         del filename_im
            #     del ske
            # del image
            # gc.collect()
    ...
    def cropAndSke(self, image, ske):
        ...
        if ske.fromImage(image):  # a skeleton is found
            ...
            ske.crop(xm / self.new_video_width, ym / new_video_height, self.ske_width / self.new_video_width,
                     self.ske_height / new_video_height)  # skeleton crop
    ...
    def load(cls, filename):
        ...
        print("VideoSkeleton::load: " + filename + " #skeleton=" + str(objet_charge.ske.shape) + " #image=" + str(
            objet_charge.im.shape))
        return objet_charge
